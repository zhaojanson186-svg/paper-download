...
tab1, tab2 = st.tabs(["📄 核心文献直传 + AI 深度精读", "💡 欧洲专利雷达 + AI 构型拆解"])

# ========================================================
# 引擎 1：文献抓取 + AI 精读报表
# ========================================================
with tab1:
    st.markdown("### 🧬 学术前沿：抓取原文并生成 AI 深度透视报表")
    # 注意这里的变量名 query_paper
                ...
                if not new_pmc_ids:
                    ...
                    for i, pmcid in enumerate(new_pmc_ids):
                        ...
                        paper_report_data.append({
                            ...
                            "🔬前沿机制/构型": ai_insights.get("前沿机制设计", ""),
                    ...
                    # 彻底修复 KeyError：直接使用 query_paper 变量
...
# ========================================================
with tab2:
    st.markdown("### 🧠 竞争对手挖掘：欧洲局直连与大模型构型拆解")
    # 注意这里的变量名 query_patent
    ...
    if st.button("📊 生成【专利 AI 提纯】报表并推送", type="primary"):
        if not query_patent or not gdrive_folder_id:
            ...
            with st.spinner("正在通过 Europe PMC 提取底层专利数据并比对账本..."):
                drive_service, err = get_gdrive_service(gcp_token)
                if not drive_service:
                    st.error(f"网盘授权失败: {err}")
                else:
                    ...
                    if not patents:
                        ...
                        st.warning(f"⚠️ 未能抓取到专利数据（HTTP {debug.get('status_code', '未知')}）。请检查网络或调试日志。")
                    else:
                        ...
                        if not new_patents:
                            ...
                            st.write(f"✅ 发现 **{len(new_patents)}** 项新专利！大模型正在逐篇进行 X 光透视拆解...")
                            ...
                            for idx, pt in enumerate(new_patents):
                                ...
                                pt["🛡️特殊工程化保护"] = ai_insights.get("特殊工程化保护", "未提取") 
                            ...
                            # 彻底修复 KeyError：直接使用 query_patent 变量
